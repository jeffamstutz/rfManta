...
bool ColorDB::getNamedColor( unsigned char result[3], const std::string &name ) {
...
bool ColorDB::test() {
  ...
  for(size_t i = 0; i < num_entries-1; i++) {
    if (string(namedColors[i+1].name) < string(namedColors[i].name)) {
      cerr << "namedColors[i="<<i<<"].name("<<namedColors[i].name<<") is not < namedColors[i+1="<<i+1<<"].name("<<namedColors[i+1].name<<")\n";
      passed_all = false;
    }
  }
