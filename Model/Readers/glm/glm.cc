...
namespace Glm {
  ...
  _glmReadMTL(GLMmodel* model, char* name)
  {
    ...
    for (i = 0; i < nummaterials; i++) {
      ...
      model->materials[i].filter[0] = 1.0;
      model->materials[i].filter[1] = 1.0;
      model->materials[i].filter[2] = 1.0;
    ...
    while(fscanf(file, "%s", buf) != EOF) {
      ...
      case 'T':
        switch(buf[1]) {
        case 'r':  // Tr
          // NOTE(thiago): I cannot find any documentation for what Tr is. I am
          // assuming that it's the amount to filter the diffuse color by, but
          // this could very well be wrong.
          fscanf(file, "%f", &model->materials[nummaterials].filter[0]);
          model->materials[nummaterials].filter[2] =
            model->materials[nummaterials].filter[1] =
            model->materials[nummaterials].filter[0];
          break;
        case 'f': //Tf
          fscanf(file, "%f %f %f",
                 &model->materials[nummaterials].filter[0],
