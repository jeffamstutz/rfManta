...
AmbientOcclusion::AmbientOcclusion(const Color& color, float cutoff_dist,
                                   int num_dirs, bool bouncen) :
  cutoff(cutoff_dist), color(color), bounce(bouncen), num_directions(num_dirs)
{
...
void AmbientOcclusion::generateDirections(int num_dir)
{
  MT_RNG rng;
...
void AmbientOcclusion::computeAmbient(const RenderContext& context,
                                      RayPacket& rays,
                                      ColorArray ambient) const
{
  ...
  for(int i = rays.begin(); i < rays.end(); ++i) {
    ...
    while ( num_sent < num_directions) {
      ...
      for (int r = start; r < end; r++) {
        if (bounce) {
          if (occlusion_rays.getMinT(r) < cutoff)
...
string AmbientOcclusion::toString() const {
  ostringstream out;
