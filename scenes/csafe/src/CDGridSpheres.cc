...
CDGridSpheres::CDGridSpheres(float* spheres, int nspheres, int nvars, int ncells,
                             int depth, Real radius, int ridx, RGBAColorMap* cmap,
                             int cidx, int xindex_) :
  barrier("CDGRidSpheres barrier"), mutex("CDGridSpheres mutex"), spheres(spheres), nspheres(nspheres), nvars(nvars),
...
void CDGridSpheres::update(int proc, int numProcs)
{
  ...
  cerr<<"  min:  ("<<min[xindex]<<", "<<min[xindex+1]<<", "<<min[xindex+2]<<")\n";
  cerr<<"  max:  ("<<max[xindex]<<", "<<max[xindex+1]<<", "<<max[xindex+2]<<")\n";
...
void CDGridSpheres::intersect(const RenderContext& context, RayPacket& rays) const
{
  ...
  SphereIntersectFcn intersectSphere = &CDGridSpheres::intersectSphereDefault;
  ...
  RayPacket& rays) const
{
  ...
  Vector n=rays.getHitPosition(i) - Vector(data[xindex], data[xindex+1], data[xindex+2]);
  n.normalize();
  rays.setNormal(i, n);
}
...
void CDGridSpheres::shade(const RenderContext& context, RayPacket& rays) const
{
...
void CDGridSpheres::lambertianShade(const RenderContext& context, RayPacket& rays, ColorArray& totalLight) const
{
  // Compute normals
...
void CDGridSpheres::computeHistogram(int index, int numBuckets, int* histValues)
{
...
void CDGridSpheres::traverse(int ray_idx, RayPacket& rays, int depth, Real tnear,
        ...
        if(!skip)
          {
            ...
            (*this.*intersectSphere)(rays, ray_idx, start + j,                                                                                
                                     Vector(data[xindex], data[xindex+1], data[xindex+2]), radius2);
#else
...
#ifdef USE_OPTIMIZED_FCNS
void CDGridSpheres::intersectSphereCOND(RayPacket& rays, int ray_idx, int idx,
...
void CDGridSpheres::intersectSphereDefault(RayPacket& rays, int ray_idx, int idx,
                                           const Vector& center,
                                           float radius2) const
{
  // if (clip(cells[idx]))
...
void CDGridSpheres::transformToLattice(const BBox& box, int& sx, int& sy, int& sz,
...
void CDGridSpheres::mapDiffuseColors(Packet<Color>& diffuse, RayPacket& rays) const
{
  for (int i=int(rays.begin()); i<int(rays.end()); ++i) {
